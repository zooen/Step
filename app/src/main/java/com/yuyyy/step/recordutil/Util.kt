package com.yuyyy.step.recordutil

/**
 * Created by feliz on 5/17/18.
 */
...
public class Util {
    ...
    companion object {
         ...
         * @return
         */
        fun parseTraceLocationList(
                list: List<AMapLocation>?): List<TraceLocation> {
        ...
        fun parseLocations(latLonStr: String): ArrayList<AMapLocation> {
            ...
            for (i in latLonStrs.indices) {
                val location = Util.parseLocation(latLonStrs[i])
                if (location != null) {
